...
static bool admin_flush_extended(PgSocket *admin, PktBuf *buf, const char *desc)
{
...
static bool admin_ready_extended(PgSocket *admin, const char *desc)
{
	PktBuf buf;
	uint8_t tmp[512];
...
static bool admin_bind_complete(PgSocket *admin) {
	PktBuf buf;
	uint8_t tmp[512];
	pktbuf_static(&buf, tmp, sizeof(tmp));
	pktbuf_write_BindComplete(&buf);
	return pktbuf_send_immediate(&buf, admin);
}

static bool admin_parse_complete(PgSocket *admin) {
	PktBuf buf;
	uint8_t tmp[512];
	pktbuf_static(&buf, tmp, sizeof(tmp));
	pktbuf_write_ParseComplete(&buf);
	return pktbuf_send_immediate(&buf, admin);
}
...
/*
 * Implements a minimal requirements for supporting extended query protocol for the
 * admin console. The procedures generated by parse command are not stored, but only
 * the query is captured within the admin socket. Once the Execute packet is issued,
...
static bool admin_handle_extended_protocol(PgSocket *admin, PktHdr *pkt) {
	...
	case 'D':
	 	/*
		 * Ideally, we should return row describe packet here.
		 * However, that would require significant modification
		 * on how admin commands are being processed and executed.
		 * We probably still need to do so to comply with the protocol standards.
		 */
	...
	case 'S':
		/* just return ready */
		skip_possibly_completely_buffered_packet(admin, pkt);
		return admin_ready_extended(admin, "EXTENDED");
	}
}
...
bool admin_handle_client(PgSocket *admin, PktHdr *pkt)
{
	...
	case 'S':
		 ...
		 */
		return admin_handle_extended_protocol(admin, pkt);
	default:
