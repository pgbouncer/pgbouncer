...
def test_ldap_auth(bouncer, openldap):
    ...
    #2 test "search+bind"
    with open(hba_conf_file, 'w') as f:
        f.write(f'host all ldapuser1 0.0.0.0/0 ldap ldapserver=127.0.0.1 ldapport={openldap.ldap_port} ldapbasedn="dc=example,dc=net"')
    bouncer.admin("reload")
    bouncer.test(user="ldapuser1", password="secret1")
    ...
    #4 test "LDAP URLs"
    with open(hba_conf_file, 'w') as f:
        f.write(f'host all ldapuser1 0.0.0.0/0 ldap ldapurl="ldap://127.0.0.1:{openldap.ldap_port}/dc=example,dc=net?uid?sub"')
    bouncer.admin("reload")
    bouncer.test(user="ldapuser1", password="secret1")
    #5 test "search filters"
    with open(hba_conf_file, 'w') as f:
        f.write(f'host all ldapuser1 0.0.0.0/0 ldap ldapserver=127.0.0.1 ldapport={openldap.ldap_port} ldapbasedn="dc=example,dc=net" ldapsearchfilter="uid=$username"')
    bouncer.admin("reload")
    bouncer.test(user="ldapuser1", password="secret1")
    ##6 test "search filters in LDAP URLs"
    with open(hba_conf_file, 'w') as f:
        f.write(f'host all ldapuser1 0.0.0.0/0 ldap ldapurl="ldap://127.0.0.1:{openldap.ldap_port}/dc=example,dc=net??sub?(|(uid=$username)(mail=$username))"')
    bouncer.admin("reload")
    bouncer.test(user="ldapuser1", password="secret1")
